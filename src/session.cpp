...
void Session::on_request_read(const system::error_code& error,
    ...
    if (!this->request_parser.message_complete) {
        return asio::async_read(
            this->client_socket, asio::buffer(*buffer),
            asio::transfer_at_least(1),
            std::bind(&Session::on_request_read, this->shared_from_this(), _1,
                      _2, buffer));
    }
    ...
    this->resolver.async_resolve(this->remote_host.name,
                                 std::to_string(this->remote_host.port),
                                 std::bind(&Session::on_remote_resolve,
                                           this->shared_from_this(), _1, _2));
}

const std::string HTTP_CERT_RESPONSE_MESSAGE =
    "HTTP/1.0 200 Connection established\r\n"
    "Content-Type: application/x-x509-ca-cert\r\n"
    "Content-Disposition: attachment; filename=GITM.crt\r\n"
    "Connection: close\r\n\r\n";

void Session::cert_file_response() {
    std::string response =
        HTTP_CERT_RESPONSE_MESSAGE +
        Cert::X509_to_string(this->root_ca_info.p_ca_pub_cert);
    ...
    if (this->is_ssl) {
        ...
        this->on_proxy_data_read(
            ...
            std::make_shared<std::vector<char>>(
                this->request_parser.raw_message),
...
Cert::CertInfo Session::get_cert_for_hostname(const std::string& hostname) {
    // Check if the certificate is already in the cache
    auto it = resigned_certificates_cache.find(hostname);
    if (it != resigned_certificates_cache.end()) {
        return it->second;
    }

    auto cert = Cert::generate_certificate(root_ca_info, hostname);

    // Add the certificate to the cache
    resigned_certificates_cache[hostname] = cert;
...
void Session::on_remote_handshake(const system::error_code& error) {
    ...
    Cert::CertInfo resigned_server_cert =
        get_cert_for_hostname(this->remote_host.name);
...
void Session::proxy_data(T_stream& from, T_stream& to, bool intercept) {
    ...
    asio::async_read(from, asio::buffer(*buffer), asio::transfer_at_least(1),
                     [self = this->shared_from_this(), parser, buffer, &from,
                      &to, intercept](auto&&... args) {
...
void Session::on_proxy_data_read(const system::error_code& error,
    ...
    if (!this->host_interception_filter.empty() &&
    ...
    if (intercept && this->intercept_cb.has_value()) {
        ...
        intercept_response_cb = [self = this->shared_from_this(),
            ...
            asio::async_write(
                ...
                [self = self->shared_from_this(), final_message](
                    auto&&... args) { self->on_proxy_data_sent(args...); });
        };
        ...
        Server::InterceptedSession intercepted_session = {
            ...
            .intercept_response_cb =
                std::make_shared<Server::TInterceptResponseCB>(
